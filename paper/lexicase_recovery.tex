...
Lexicase selection can maintain high diversity and can quickly diversify low-diversity populations.

Why does lexicase do so much better at diversification??? Does it have to do with specialists? Probably! Does it have to do with concentrating on individual test cases or combinations thereof? Probabily! Also: if many individuals share the same error vector (or similar), they are competing for the same selections when using lexicase selection. On the other hand, with tournament selection, if they have good total error then they might all be selected often.
